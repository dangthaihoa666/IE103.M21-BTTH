...
from QuanLySV
where MSDH = 1
--Lệnh lấy sinh viên ở vị trí cuối cùng ở trường CNTT:
...
--Câu 9: Viết lệnh Xquery thay đổi tên sinh viên thứ 2 thành tên ‘Binh’ trong trường CNTT.
update QuanLySV
set ChiTietSV.modify('replace value of (//sinhvien/@Ten)[2] with "Binh" ')
where MSDH = 1

--Câu 10: Viết lệnh Xquery kiểm tra xem có tồn tại sinh viên có ID là 12 trong trường KHTN không? (Nếu có trả về 1, nếu không thì trả về 0). 
--Gợi ý: Dùng lệnh exist() thay cho query().
select ChiTietSV.exist('//sinhvien[@ID=12]')
from QuanLySV
where TenDH = 'DH KHTN'

--Câu 11*: Viết lệnh Xquery kiểm tra xem có tồn tại sinh viên tên ‘Lan’ trong trường CNTT không? (Nếu có trả về 1, nếu không thì trả về 0). Và INSERT thêm vào THONGTINSV:
